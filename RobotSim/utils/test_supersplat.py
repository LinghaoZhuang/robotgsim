...
if __name__ == "__main__":
    ...
    translation = np.array([0.34, 0.09, 0.42])  # x, y, z平移
    # translation = np.array([-0, -0.0, 0.])
    rotation_degrees = np.array([-34.29, 11.67, -180-47.35])  # x, y, z旋转（度）
    
    # rotation_degrees = np.array([-34.29, 11.67, -180-47.35])
    transform = torch.tensor(build_transform(translation, rotation_degrees, degrees=True, s=0.81, order="xyz"))
    # blue A, Red B
    ...
    # print('le')
    visualize_point_clouds_o3d(lerobot_sim, lerobot_real, transform, window_name="ICP Registration Result")